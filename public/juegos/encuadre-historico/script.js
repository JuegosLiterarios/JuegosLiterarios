(async () => {
  ...
  const LS_KEY = "jl_encuadre_historico_v1";
  ...
  function mostrarPregunta() {
    ...
    $("#autorNombre").textContent = item.autor;
    $("#autorAnos").textContent = item.anos;

    opcionesEl.innerHTML = "";
    const letras = ["A", "B", "C", "D"];
    JL.shuffle([item.evento, ...item.distractores]).forEach((texto, i) => {
  ...
  function responder(item, texto, btnClickeado) {
    ...
    opcionesEl.querySelectorAll(".opcion").forEach((b) => {
      b.disabled = true;
      if (b.querySelector("span:last-child").textContent === item.evento) b.classList.add("correcta");
    });

    const gano = texto === item.evento;
    if (gano) {
      racha++;
      aciertosPartida++;
    ...
    explicacionEl.innerHTML = `📜 <b>${item.autor}</b> vivió entre ${item.anos} · ${item.evento}.`;
  ...
  function finPartida() {
    ...
    const msg = puntos >= 150 ? "¡Historiador de la literatura! 🏆" :
                puntos >= 110 ? "Ubicas a cada autor en su siglo 🧭" :
                puntos >= 70 ? "Buena brújula histórica 🌱" : "La historia te dará revancha";
