(async () => {
  ...
  const LS_KEY = "jl_reconoce_dialogo_v1";
  ...
  const res = await fetch("dialogos.json");
  ...
  function iniciarPartida() {
    const cantidad = Math.min(TOTAL_PREGUNTAS, banco.length);
    partida = JL.shuffle(banco).slice(0, cantidad);
    indice = 0;
    puntos = 0;
    racha = 0;
    aciertosPartida = 0;
  ...
  function mostrarPregunta() {
    ...
    const consigna = item.tipo === "libro" ? "¿De qué libro es esta línea?" : "¿Quién dice esta línea?";
    promptEl.innerHTML =
      `<blockquote class="cita-literaria">«${item.texto}»</blockquote>
       <p class="pregunta-consigna">${consigna}</p>`;
  ...
  function finPartida() {
    ...
    const msg = puntos >= 150 ? "¡Oído absoluto para las grandes líneas! 🏆" :
                puntos >= 110 ? "Reconoces la voz de los clásicos 📖" :
                puntos >= 70 ? "Buen radar de lector 🌱" : "Estas líneas piden otra ronda";
